...
void fft::powerSpectrum(int start, int half, float *data, int windowSize,float *magnitude,float *phase, float *power, float *max_mag, float *avg_power) {
    int i;
    int windowFunc = 3;
    ...
    for (i = 0; i < half; i++) {
        ...
        if(tempMag < magnitude[i]) tempMag = magnitude[i];
    ...
    *(max_mag) = tempMag;
    //printf("avg_power: %f total_power: %f max_mag: %f \n", *avg_power, total_power, *max_mag);
